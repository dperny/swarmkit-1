...
type Task struct {
	...
	// Devices represents the actual device attachment instructions for a
	// particular task.
	Devices []*DeviceAttachment `protobuf:"bytes,16,rep,name=devices" json:"devices,omitempty"`
}
...
type DeviceClass struct {
	ID   string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Meta Meta   `protobuf:"bytes,2,opt,name=meta" json:"meta"`
	// Spec contains the user defined spec of this device class, including the
	// name of the device class and any configuration options the user has set.
	Spec DeviceClassSpec `protobuf:"bytes,3,opt,name=spec" json:"spec"`
}

func (m *DeviceClass) Reset()                    { *m = DeviceClass{} }
func (*DeviceClass) ProtoMessage()               {}
func (*DeviceClass) Descriptor() ([]byte, []int) { return fileDescriptorObjects, []int{10} }
...
func (*Resource) Descriptor() ([]byte, []int) { return fileDescriptorObjects, []int{11} }
...
func (*Extension) Descriptor() ([]byte, []int) { return fileDescriptorObjects, []int{12} }
...
func init() {
	...
	proto.RegisterType((*DeviceClass)(nil), "docker.swarmkit.v1.DeviceClass")
...
func (m *Task) CopyFrom(src interface{}) {
	...
	*m = *o
	...
	if o.Devices != nil {
		m.Devices = make([]*DeviceAttachment, len(o.Devices))
		for i := range m.Devices {
			m.Devices[i] = &DeviceAttachment{}
			deepcopy.Copy(m.Devices[i], o.Devices[i])
		}
	}
...
func (m *DeviceClass) Copy() *DeviceClass {
	if m == nil {
		return nil
	}
	o := &DeviceClass{}
...
func (m *DeviceClass) CopyFrom(src interface{}) {
...
type DeviceClassCheckFunc func(t1, t2 *DeviceClass) bool

type EventDeviceClass interface {
	IsEventDeviceClass() bool
}

type EventCreateDeviceClass struct {
	DeviceClass *DeviceClass
	Checks      []DeviceClassCheckFunc
}

func (e EventCreateDeviceClass) Matches(apiEvent go_events.Event) bool {
	typedEvent, ok := apiEvent.(EventCreateDeviceClass)
	if !ok {
		return false
	}

	for _, check := range e.Checks {
		if !check(e.DeviceClass, typedEvent.DeviceClass) {
			return false
		}
	}
	return true
...
func (e EventCreateDeviceClass) IsEventCreate() bool {
	return true
}

func (e EventCreateDeviceClass) IsEventDeviceClass() bool {
	return true
}

type EventUpdateDeviceClass struct {
	DeviceClass    *DeviceClass
	OldDeviceClass *DeviceClass
	Checks         []DeviceClassCheckFunc
}

func (e EventUpdateDeviceClass) Matches(apiEvent go_events.Event) bool {
	typedEvent, ok := apiEvent.(EventUpdateDeviceClass)
	if !ok {
		return false
	}
...
func (e EventUpdateDeviceClass) IsEventUpdate() bool {
	return true
}

func (e EventUpdateDeviceClass) IsEventDeviceClass() bool {
	return true
}

type EventDeleteDeviceClass struct {
	DeviceClass *DeviceClass
	Checks      []DeviceClassCheckFunc
}

func (e EventDeleteDeviceClass) Matches(apiEvent go_events.Event) bool {
	typedEvent, ok := apiEvent.(EventDeleteDeviceClass)
	if !ok {
		return false
	}
...
func (e EventDeleteDeviceClass) IsEventDelete() bool {
	return true
}

func (e EventDeleteDeviceClass) IsEventDeviceClass() bool {
	return true
}

func (m *DeviceClass) CopyStoreObject() StoreObject {
	return m.Copy()
}

func (m *DeviceClass) GetMeta() Meta {
	return m.Meta
}

func (m *DeviceClass) SetMeta(meta Meta) {
	m.Meta = meta
}

func (m *DeviceClass) GetID() string {
	return m.ID
}

func (m *DeviceClass) EventCreate() Event {
	return EventCreateDeviceClass{DeviceClass: m}
}

func (m *DeviceClass) EventUpdate(oldObject StoreObject) Event {
	if oldObject != nil {
		return EventUpdateDeviceClass{DeviceClass: m, OldDeviceClass: oldObject.(*DeviceClass)}
	} else {
		return EventUpdateDeviceClass{DeviceClass: m}
	}
}

func (m *DeviceClass) EventDelete() Event {
	return EventDeleteDeviceClass{DeviceClass: m}
}

func DeviceClassCheckID(v1, v2 *DeviceClass) bool {
	return v1.ID == v2.ID
}

func DeviceClassCheckIDPrefix(v1, v2 *DeviceClass) bool {
	return strings.HasPrefix(v2.ID, v1.ID)
}

func DeviceClassCheckName(v1, v2 *DeviceClass) bool {
	return v1.Spec.Annotations.Name == v2.Spec.Annotations.Name
}

func DeviceClassCheckNamePrefix(v1, v2 *DeviceClass) bool {
	return strings.HasPrefix(v2.Spec.Annotations.Name, v1.Spec.Annotations.Name)
}

func DeviceClassCheckCustom(v1, v2 *DeviceClass) bool {
	return checkCustom(v1.Spec.Annotations, v2.Spec.Annotations)
}

func DeviceClassCheckCustomPrefix(v1, v2 *DeviceClass) bool {
	return checkCustomPrefix(v1.Spec.Annotations, v2.Spec.Annotations)
}

func ConvertDeviceClassWatch(action WatchActionKind, filters []*SelectBy) ([]Event, error) {
	var (
		m          DeviceClass
		checkFuncs []DeviceClassCheckFunc
	)
	...
	for _, filter := range filters {
		...
		case *SelectBy_ID:
			...
			m.ID = v.ID
			checkFuncs = append(checkFuncs, DeviceClassCheckID)
		case *SelectBy_IDPrefix:
			if m.ID != "" {
				return nil, errConflictingFilters
			}
			m.ID = v.IDPrefix
			checkFuncs = append(checkFuncs, DeviceClassCheckIDPrefix)
		case *SelectBy_Name:
			if m.Spec.Annotations.Name != "" {
				return nil, errConflictingFilters
			}
			m.Spec.Annotations.Name = v.Name
			checkFuncs = append(checkFuncs, DeviceClassCheckName)
		case *SelectBy_NamePrefix:
			if m.Spec.Annotations.Name != "" {
				return nil, errConflictingFilters
			}
			m.Spec.Annotations.Name = v.NamePrefix
			checkFuncs = append(checkFuncs, DeviceClassCheckNamePrefix)
		case *SelectBy_Custom:
			if len(m.Spec.Annotations.Indices) != 0 {
				return nil, errConflictingFilters
			}
			m.Spec.Annotations.Indices = []IndexEntry{{Key: v.Custom.Index, Val: v.Custom.Value}}
			checkFuncs = append(checkFuncs, DeviceClassCheckCustom)
		case *SelectBy_CustomPrefix:
			if len(m.Spec.Annotations.Indices) != 0 {
				return nil, errConflictingFilters
			}
			m.Spec.Annotations.Indices = []IndexEntry{{Key: v.CustomPrefix.Index, Val: v.CustomPrefix.Value}}
			checkFuncs = append(checkFuncs, DeviceClassCheckCustomPrefix)
		}
	}
	var events []Event
	if (action & WatchActionKindCreate) != 0 {
		events = append(events, EventCreateDeviceClass{DeviceClass: &m, Checks: checkFuncs})
	}
	if (action & WatchActionKindUpdate) != 0 {
		events = append(events, EventUpdateDeviceClass{DeviceClass: &m, Checks: checkFuncs})
	}
	if (action & WatchActionKindRemove) != 0 {
		events = append(events, EventDeleteDeviceClass{DeviceClass: &m, Checks: checkFuncs})
	}
	if len(events) == 0 {
		return nil, errUnrecognizedAction
	}
...
type DeviceClassIndexerByID struct{}

func (indexer DeviceClassIndexerByID) FromArgs(args ...interface{}) ([]byte, error) {
	return fromArgs(args...)
}
func (indexer DeviceClassIndexerByID) PrefixFromArgs(args ...interface{}) ([]byte, error) {
	return prefixFromArgs(args...)
}
func (indexer DeviceClassIndexerByID) FromObject(obj interface{}) (bool, []byte, error) {
	m := obj.(*DeviceClass)
	return true, []byte(m.ID + "\x00"), nil
}

type DeviceClassIndexerByName struct{}

func (indexer DeviceClassIndexerByName) FromArgs(args ...interface{}) ([]byte, error) {
	return fromArgs(args...)
}
func (indexer DeviceClassIndexerByName) PrefixFromArgs(args ...interface{}) ([]byte, error) {
	return prefixFromArgs(args...)
}
func (indexer DeviceClassIndexerByName) FromObject(obj interface{}) (bool, []byte, error) {
	m := obj.(*DeviceClass)
	val := m.Spec.Annotations.Name
	return true, []byte(strings.ToLower(val) + "\x00"), nil
}

type DeviceClassCustomIndexer struct{}

func (indexer DeviceClassCustomIndexer) FromArgs(args ...interface{}) ([]byte, error) {
	return fromArgs(args...)
}
func (indexer DeviceClassCustomIndexer) PrefixFromArgs(args ...interface{}) ([]byte, error) {
	return prefixFromArgs(args...)
}
func (indexer DeviceClassCustomIndexer) FromObject(obj interface{}) (bool, [][]byte, error) {
	m := obj.(*DeviceClass)
	return customIndexer("", &m.Spec.Annotations)
}
...
func NewStoreAction(c Event) (StoreAction, error) {
	...
	case EventDeleteConfig:
		sa.Action = StoreActionKindRemove
		sa.Target = &StoreAction_Config{Config: v.Config}
	case EventCreateDeviceClass:
		sa.Action = StoreActionKindCreate
		sa.Target = &StoreAction_DeviceClass{DeviceClass: v.DeviceClass}
	case EventUpdateDeviceClass:
		sa.Action = StoreActionKindUpdate
		sa.Target = &StoreAction_DeviceClass{DeviceClass: v.DeviceClass}
	case EventDeleteDeviceClass:
		sa.Action = StoreActionKindRemove
		sa.Target = &StoreAction_DeviceClass{DeviceClass: v.DeviceClass}
...
func EventFromStoreAction(sa StoreAction, oldObject StoreObject) (Event, error) {
	...
	case *StoreAction_DeviceClass:
		switch sa.Action {
		case StoreActionKindCreate:
			return EventCreateDeviceClass{DeviceClass: v.DeviceClass}, nil
		case StoreActionKindUpdate:
			if oldObject != nil {
				return EventUpdateDeviceClass{DeviceClass: v.DeviceClass, OldDeviceClass: oldObject.(*DeviceClass)}, nil
			} else {
				return EventUpdateDeviceClass{DeviceClass: v.DeviceClass}, nil
			}
		case StoreActionKindRemove:
			return EventDeleteDeviceClass{DeviceClass: v.DeviceClass}, nil
		}
...
func WatchMessageEvent(c Event) *WatchMessage_Event {
	...
	case EventCreateDeviceClass:
		return &WatchMessage_Event{Action: WatchActionKindCreate, Object: &Object{Object: &Object_DeviceClass{DeviceClass: v.DeviceClass}}}
	case EventUpdateDeviceClass:
		if v.OldDeviceClass != nil {
			return &WatchMessage_Event{Action: WatchActionKindUpdate, Object: &Object{Object: &Object_DeviceClass{DeviceClass: v.DeviceClass}}, OldObject: &Object{Object: &Object_DeviceClass{DeviceClass: v.OldDeviceClass}}}
		} else {
			return &WatchMessage_Event{Action: WatchActionKindUpdate, Object: &Object{Object: &Object_DeviceClass{DeviceClass: v.DeviceClass}}}
		}
	case EventDeleteDeviceClass:
		return &WatchMessage_Event{Action: WatchActionKindRemove, Object: &Object{Object: &Object_DeviceClass{DeviceClass: v.DeviceClass}}}
	case EventCreateResource:
...
func ConvertWatchArgs(entries []*WatchRequest_WatchEntry) ([]Event, error) {
	...
	for _, entry := range entries {
		...
		case "deviceclass":
			newEvents, err = ConvertDeviceClassWatch(entry.Action, entry.Filters)
		default:
...
func (this *Task) String() string {
	...
	s := strings.Join([]string{`&Task{`,
		...
		`Devices:` + strings.Replace(fmt.Sprintf("%v", this.Devices), "DeviceAttachment", "DeviceAttachment", 1) + `,`,
		`}`,
	}, "")
	return s
}
...
func (m *Task) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 16:
			...
			m.Devices = append(m.Devices, &DeviceAttachment{})
...
var fileDescriptorObjects = []byte{
	...
	,
}
